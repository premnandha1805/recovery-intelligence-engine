...
1.  Chronological billing cycles:
        For each customer x subscription pair, the simulator steps through
...
---------------------------------
    ...
    cumulative_failures         # prior failed cycles for this customer
    consecutive_failed_cycles   # currently ongoing streak of failed cycles
    ...
    notification_engagement_score  Observable proxy for nudge responsiveness
    contact_response_score         Observable proxy for retry responsiveness
"""
...
def generate_payments(
    ...
    for _, record in merged.iterrows():
        ...
        consecutive_failed_cycles = 0   # resets to 0 after any successful cycle

        # Per-customer proxy scores (stable across all attempt rows)
        notif_score   = float(record["notification_engagement_score"])
        contact_score = float(record["contact_response_score"])
        ...
        for cycle_idx in range(n_billing_cycles):
            ...
            if not _cycle_fails(dynamic_success_rate, rng):
                ...
                consecutive_failed_cycles = 0   # streak broken
                continue
            ...
            for attempt in range(1, MAX_ATTEMPTS_PER_CYCLE + 1):
                ...
                rows.append({
                    ...
                    # Per-customer observable proxies (constant within customer)
                    "notification_engagement_score":  notif_score,
                    "contact_response_score":         contact_score,
                })
